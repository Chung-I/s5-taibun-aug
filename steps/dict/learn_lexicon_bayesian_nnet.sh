...
if [ $stage -le 0 ]; then
  ...
  if [ -z $lexiconp_g2p ]; then
    ...
    #cat $lexiconp_g2p | awk '{if (NF<2) {print "There is an empty pronunciation in lexicon_g2p.txt. Exit." \
    ...
    cat $dir/lexiconp_g2p.txt | cut -d ' ' -f2,3- > $dir/lexicon_g2p.txt 2>/dev/null
  fi
fi
...
if [ $stage -le 1 ] && $retrain_src_mdl; then
  ...
  echo $oov_symbol
  utils/prepare_lang.sh --position-dependent-phones false --phone-symbol-table $ref_lang/phones.txt \
    $dir/dict_expanded_target_vocab \
...
if [ $stage -le 2 ]; then
  ...
  utils/prepare_lang.sh  --position-dependent-phones false \
    --phone-symbol-table $ref_lang/phones.txt $dir/dict_expanded_train "$oov_symbol" \
...
if [ $stage -le 3 ]; then
  ...
  steps/cleanup/debug_lexicon_nnet.sh --nj $nj --cmd "$decode_cmd"  --stage 6 \
    $data $dir/lang_expanded_train \
...
if [ $stage -le 4 ]; then
  ...
  if $retrain_src_mdl; then mdl_dir=$dir/${src_mdl_dir}_retrained; else mdl_dir=$src_mdl_dir; fi
  steps/chain/align_lats.sh --cmd "$decode_cmd" --nj $nj --generate-ali-from-lats true \
    --online-ivector-dir exp/nnet3/ivectors_${base_data} \
...
if [ $stage -le 5 ]; then
  ...
  if $retrain_src_mdl; then mdl_dir=$dir/${src_mdl_dir}_retrained; else mdl_dir=$src_mdl_dir; fi
  steps/chain/align_lats.sh --cmd "$decode_cmd" --nj $nj \
    --online-ivector-dir exp/nnet3/ivectors_${base_data} \
