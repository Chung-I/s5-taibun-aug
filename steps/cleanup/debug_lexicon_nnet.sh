...
if [ -z $alidir ]; then
  ...
  if [ $stage -le 1 ]; then
    ...
    echo $base_data
    steps/chain/align_lats.sh --cmd "$cmd" --nj $nj --generate-ali-from-lats true \
      --online-ivector-dir exp/nnet3/ivectors_${base_data} \
      $data $lang $src $alidir  || exit 1;
  fi
fi
...
if [ $stage -le 4 ]; then
  data_name="$(basename $data)"
  base_data="${data_name%_hires}"
  steps/nnet3/decode.sh  --skip-scoring true --acwt 1.0 --post-decode-acwt 10.0 \
    --nj $nj --cmd "$cmd" \
    --online-ivector-dir exp/nnet3/ivectors_${base_data} \
    $src/graph_phone_bg ${data} $src/decode_${split}_phone_bg || exit 1;
fi

if [ $stage -le 5 ]; then
  steps/get_train_ctm.sh --print-silence true --use-segments false --frame-shift 0.03 \
     --cmd "$cmd" $data $lang $alidir
fi

if [ $stage -le 6 ]; then
  steps/get_ctm.sh --use-segments false --cmd "$cmd" --min-lmwt 3 --max-lmwt 8 --frame-shift 0.03 \
     $data $src/graph_phone_bg $src/decode_$(basename $data)_phone_bg
fi
