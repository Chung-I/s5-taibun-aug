...
stage=0
train_set=train
test_sets="test_pts_merged test eval"
gmm=tri5a
...
if [ $stage -le 3 ]; then
  local/nnet3/run_aug.sh || exit 1;
  utils/combine_data.sh data/${train_set}_aug_sp_hires_nopitch "data/${train_set}_sp_hires_nopitch data/${train_set}_aug_hires_nopitch"
  utils/combine_data.sh data/${train_set}_aug_sp_hires "data/${train_set}_sp_hires data/${train_set}_aug_hires"
  utils/combine_data.sh data/${train_set}_aug_sp "data/${train_set}_sp data/${train_set}_aug"
  steps/combine_ali_dirs.sh --nj 12 data/${train_set}_aug_sp exp/tri5a_aug_sp_ali exp/tri5a_sp_ali exp/tri5a_aug_ali || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  utils/data/subset_data_dir.sh data/${train_set}_aug_sp_hires_nopitch \
     $num_utts ${temp_data_root}/${train_set}_aug_sp_hires_nopitch_subset

  echo "$0: computing a PCA transform from the hires data."
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
      --splice-opts "--left-context=3 --right-context=3" \
      --max-utts 10000 --subsample 2 \
       ${temp_data_root}/${train_set}_aug_sp_hires_nopitch_subset \
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    --num-frames 700000 \
    ${temp_data_root}/${train_set}_aug_sp_hires_nopitch_subset 512 \
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
     data/${train_set}_aug_sp_hires_nopitch exp/nnet3${nnet3_affix}/diag_ubm \
...
if [ $stage -le 6 ]; then
  ...
  ivectordir=exp/nnet3${nnet3_affix}/ivectors_${train_set}_aug_sp

  # having a larger number of speakers is helpful for generalization, and to
  # handle per-utterance decoding well (iVector starts at zero).
  temp_data_root=${ivectordir}
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/${train_set}_aug_sp_hires_nopitch ${temp_data_root}/${train_set}_aug_sp_hires_nopitch_max2
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
    ${temp_data_root}/${train_set}_aug_sp_hires_nopitch_max2 \
