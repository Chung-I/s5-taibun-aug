...
cat $train_dir/transcript.csv | awk -F ',' '{print $1}' > tmp/wavlist
cat $train_dir/transcript.csv | awk -F ',' '{print $1}' | cut -d '.' -f1 > tmp/uttlist
sed -i "s/^/${drama_name}-/g" tmp/uttlist
sed -i "s|^|${train_dir}/segment/|g" tmp/wavlist
paste -d " " tmp/uttlist tmp/wavlist > $data_dir/wav.scp
cat $train_dir/transcript.csv | awk -F ',' '{print $2}' > tmp/only_text
