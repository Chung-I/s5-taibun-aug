...
dir=${dir}${affix:+_$affix}_aug_sp
train_set=train_aug_sp
ali_dir=exp/tri5a_aug_sp_ali
...
# alignments for it.
echo $test_sets
local/nnet3/run_ivector_aug_common.sh --stage $stage --train-set train --gmm tri5a ${nnet3_affix:+ --nnet3-affix $nnet3_affix} \
  --test-sets "$test_sets"
...
if [ $stage -le 7 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" data/$train_set \
    data/lang exp/tri5a exp/tri5a_aug_sp_lats
  rm exp/tri5a_aug_sp_lats/fsts.*.gz # save space
fi
...
if [ $stage -le 13 ]; then
  for test_set in $test_sets; do
    numspk=$(wc -l <data/${test_set}/spk2utt)
    nj=$([ $numspk -le $num_cpu_jobs ] && echo "$numspk" || echo "$num_jobs")
    #nj=12
