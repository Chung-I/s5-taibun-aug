...
parser.add_argument("--output-name", nargs='+', default=['output', 'output-xent'])
...
for i, output in enumerate(args.output_name):
    old_output = output
    output = '\'' + output + '\''
    ...
    plt.plot(train[0], train[1], 'C'+str(2*i), label=f"{old_output} Train")
    plt.plot(valid[0], valid[1], 'C'+str((2*i)+1), label=f"{old_output} Dev")
